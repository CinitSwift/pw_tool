...
export function RunningState({ snapshot, api }: TimerStateProps) {
  ...
  return (
    <section className="timer-state" aria-labelledby="running-title">
      ...
      <div className="timer-display"><span>有效时长</span><strong>{formatDuration(snapshot.runtime.effectiveSeconds)}</strong><div className="time-facts"><span>开始 {formatLocalTime(firstStart)}</span><span>本段已计 {formatDuration(Math.max(0, snapshot.runtime.effectiveSeconds - closedSeconds))}</span><span>当前片段 第 {snapshot.runtime.currentSegmentIndex === null ? 0 : snapshot.runtime.currentSegmentIndex + 1} 段</span></div></div>
