...
describe('session state machine', () => {
  ...
  it('trims notes and counts Unicode code points, without changing fee', () => {
    const session = runningAt(0);
    const note = `  ${'😀'.repeat(500)}  `;
    const updated = updateSessionNote(session, note);

    expect(updated.note).toBe('😀'.repeat(500));
    expect(updated.fee).toEqual(session.fee);
    ...
    expect(updateSessionNote(completeSession(session, 60_000), ' done ').note).toBe('done');
  });
